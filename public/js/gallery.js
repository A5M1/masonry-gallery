!(function () {
	...
	function observeLazyElements(selector, handler, margin) {
		...
		document
			.querySelectorAll(`${selector}:not([data-observed])`)
			.forEach(el => {
				observer.observe(el);
		...
		$.get(
			...
			html => {
				try {
					...
					if (!fragment)
						return console.warn(
							"No masonry fragment returned from server"
						);

					const nextHasMore =
						fragment.getAttribute("data-hasmore") === "1";
			...
			afterShow: (inst, obj) => {
				...
				if (video) {
					...
					if (srcEl && !srcEl.src)
						srcEl.src =
							srcEl.dataset.src ||
							srcEl.getAttribute("data-src") ||
							srcEl.src;
					Object.assign(video, {
						controls: true,
						preload: "metadata",
						volume: galleryVolume,
						muted: false,
						loop: true
					});
					video.load();
		...
		document.addEventListener("DOMContentLoaded", () => {
			fetch("/api/tree")
				...
				.then(data => {
					...
					document
						.querySelectorAll(".folder-item.has-children")
						.forEach(item => {
					...
					if (active) {
						...
						while (
							parent &&
							parent.classList &&
							parent.classList.contains("folder-item")
						) {
