						...
						loadFolders()
							.then(() => {
								try {
									...
									if (!container) return;
									const folderEls =
										container.querySelectorAll(".folder");
									const normNew = normalizePath(newPath);
									const newLast =
										normNew && normNew.split("/").length
											? normNew.split("/").pop()
											: "";
									for (const el of folderEls) {
										const elp = normalizePath(
											el.dataset.full || ""
										);
										if (
											elp === normNew ||
									...
									console.warn(
										"Failed to select new folder after WS event",
										e
									);
								}
							})
